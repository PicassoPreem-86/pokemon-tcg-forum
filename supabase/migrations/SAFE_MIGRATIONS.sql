-- ================================================
-- SAFE MIGRATIONS FOR POKEMON TCG FORUM
-- Handles cases where objects already exist
-- ================================================
...
-- BOOKMARKS TABLE (already exists, skip)
...
-- Add deleted_by field to threads and replies
ALTER TABLE threads
...
CREATE POLICY "Admins can view all rate limits" ON rate_limits
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.id = auth.uid()
...
BEGIN
  ...
  IF v_record IS NULL OR v_now - v_record.window_start >= v_window_duration THEN
    ...
    DO UPDATE SET count = 1, window_start = v_now, updated_at = v_now;
